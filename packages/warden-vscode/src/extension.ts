    ...
    vscode.commands.registerCommand("vizVibe.recordTurn", async () => {
      ...
      if (!mmdExists) {
        vscode.window.showWarningMessage(
          'vizvibe.mmd not found. Run "Viz Vibe: Initialize Project" first.',
        );
        return;
      }
...
workspacePath: ${workspacePath}`;
      ...
      vscode.window.showInformationMessage(
        "📋 Viz Vibe: Update request copied to clipboard. Paste in AI chat to update trajectory.",
      );
    }),
  );
...
async function initializeVizVibe(
...
async function setupCursorRules(workspaceRoot: vscode.Uri) {
  ...
  const appHost = (
    (vscode.env as unknown as { appHost?: string }).appHost ?? ""
  ).toLowerCase();
  ...
  const extensionPath =
    vscode.extensions.getExtension("viz-vibe.viz-vibe")?.extensionPath;
