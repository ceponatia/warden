...
describe("TrajectoryStore", () => {
  ...
  it("should detect cycles in patch", async () => {
    ...
    const cyclePatch: PatchOperation[] = [
      {
        type: "addEdge",
        edge: { from: "n2", to: "n1", kind: "blocks", metadata: {} },
      },
    ];
    await expect(store.patch("test-actor", cyclePatch)).rejects.toThrow(
      /Cycle detected/,
    );
  });
  ...
  it("should enforce optimistic concurrency", async () => {
    ...
    await expect(store.patch("actor-2", [], 0)).rejects.toThrow(
