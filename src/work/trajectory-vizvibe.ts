...
// Rationale: parsing supports mixed Mermaid constructs and metadata extraction in one pass for deterministic import behavior.
// eslint-disable-next-line complexity
export function parseMermaidTrajectory(
  mmd: string,
  repoSlug: string,
): TrajectoryGraph {
  ...
  const edgeRegex =
    /^\s*([a-zA-Z0-9_-]+)\s*(-{2,}|-\.{1,}-)>\s*([a-zA-Z0-9_-]+)/;
  ...
  for (const line of lines) {
    ...
    if (metaMatch && metaMatch[1] && metaMatch[2]) {
      ...
      metaMap.set(metaMatch[1], {
        type: parts[0] || "task",
        status: (parts[1] === "closed"
          ? "closed"
          : "opened") as TrajectoryNodeStatus,
        date: parts[2],
        author: parts[3],
      });
      continue;
...
// Rationale: export centralizes node metadata, edges, and visual styling to preserve stable round-trip output.
// eslint-disable-next-line complexity
