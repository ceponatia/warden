...
export const TrajectoryNodeStatusSchema = z.enum([
  "opened",
  "closed",
  "blocked",
  "deferred",
...
export const TrajectoryEdgeSchema = z.object({
  ...
  to: z.string(),
  kind: z
    .enum(["dependsOn", "relatesTo", "supersedes", "blocks", "planned"])
    .default("blocks"),
  metadata: z.record(z.string(), z.any()).default({}),
});
...
export type PatchOperation =
  ...
  | {
      type: "updateNode";
      id: string;
      updates: Omit<Partial<TrajectoryNode>, "id" | "createdAt" | "updatedAt">;
    }
...
export const PatchOperationSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    id: z.string(),
    updates: TrajectoryNodeSchema.partial().omit({
      id: true,
      createdAt: true,
      updatedAt: true,
    }),
